...
async function deleteDrug() {
  ...
  finishActionModal(true, relatedPrescriptions.length
...
document.addEventListener("click", event => {
  ...
  if (auditMenuBtn) {
    const menuId = `audit_menu_${auditMenuBtn.dataset.auditMenu}`;
    document.querySelectorAll(".audit-menu").forEach(menu => { if (menu.id !== menuId) menu.classList.add("hidden"); });
    q(menuId)?.classList.toggle("hidden");
    return;
  }

  const auditBtn = event.target.closest(".audit-btn");
  if (auditBtn) {
    document.querySelectorAll(".audit-menu").forEach(menu => menu.classList.add("hidden"));
...
q("inventoryLocationFilter").onchange = renderInventory;
