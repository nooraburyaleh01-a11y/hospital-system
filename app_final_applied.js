...
const selectedMonthKey = () => {
  const year = Number(APP.cache.settings.year || jordanNowYear());
  const monthIndex = Math.max(0, MONTHS.indexOf(APP.cache.settings.month || jordanNowMonthName()));
...
function jordanNowYear() {
  return Number(jordanDateParts().year);
}

function jordanNowMonthName() {
  return MONTHS[Number(jordanDateParts().month) - 1] || MONTHS[0];
}
...
function msUntilEditExpires(rx) {
  const created = parseJordanDateTime(rx?.dateTime);
  if (!created) return 0;
  return created.getTime() + 60 * 60 * 1000 - Date.now();
}
...
function canEditPrescription(rx) {
  ...
  return msUntilEditExpires(rx) > 0;
}
...
function renderLiveClocks() {
  ...
  document.querySelectorAll('[data-edit-btn]').forEach(btn => {
    ...
    if (!rx) return;
    const allowed = canEditPrescription(rx);
    btn.classList.toggle('hidden', !allowed);
    const countdown = document.querySelector(`[data-edit-countdown="${rx.id}"]`);
    if (!countdown) return;
    if (APP.currentRole === 'ADMIN') {
      const ms = msUntilEditExpires(rx);
      countdown.textContent = ms > 0 ? `Remaining ${formatCountdown(ms)}` : 'Admin override';
    } else if (allowed) {
      countdown.textContent = `Remaining ${formatCountdown(msUntilEditExpires(rx))}`;
    } else {
      countdown.textContent = 'Edit time ended';
    }
  });
}
...
function renderStaticOptions() {
  ...
  if (q("resetPasswordPharmacy")) q("resetPasswordPharmacy").innerHTML = WORK_PHARMACIES.map(name => `<option value="${esc(name)}">${esc(name)}</option>`).join("");
...
async function submitConfirmedAction() {
  ...
  const { type, id } = APP.confirmAction;
...
function renderSettings() {
  ...
  q("settingsMonth").value = APP.cache.settings.month || jordanNowMonthName();
  q("settingsYear").value = APP.cache.settings.year || jordanNowYear();
  ...
  }).join("") || `<tr><td colspan="6" class="empty-state">No pharmacists found.</td></tr>`;
}
...
function findDuplicatePrescriptions(payload) {
  const now = Date.now();
  return APP.cache.prescriptions.filter(row => row.drugId === payload.drugId && WORK_PHARMACIES.includes(row.pharmacy)).filter(row => {
    const rowDate = parseJordanDateTime(row.dateTime);
    if (!rowDate) return false;
    const within30 = now - rowDate.getTime() <= 30 * 24 * 60 * 60 * 1000;
    const samePatient = String(row.patientName || "").trim().toLowerCase() === String(payload.patientName || "").trim().toLowerCase();
    return within30 && samePatient;
  }).sort((a,b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));
}

function openDuplicateWarningModal(payload, duplicates) {
  APP.pendingQuickPayload = payload;
  q("duplicateWarningText").textContent = "Similar prescriptions were found for the same patient and drug within the last 30 days.";
  q("duplicateWarningTbody").innerHTML = duplicates.map(row => {
    const drug = APP.cache.drugs.find(d => d.id === row.drugId);
    return `<tr><td>${esc((drug?.tradeName || "") + " " + (drug?.strength || ""))}</td><td>${esc(row.patientName || "")}</td><td>${esc(row.fileNumber || "")}</td><td>${esc(row.pharmacy || "")}</td><td>${esc(formatJordanDateTime(row.dateTime))}</td></tr>`;
  }).join("");
  openModal("duplicateWarningModal");
}

async function continueQuickRegistration(payload) {
  const { drugId, patientName, fileNumber, doctorName, pharmacistName, qtyBoxes, qtyUnits } = payload;
  const drug = APP.cache.drugs.find(d => d.id === drugId);
  ...
  try {
    ...
    await setDoc(prescriptionRef, {
      drugId, pharmacy, patientName, fileNumber, doctorName, pharmacistName, qtyBoxes, qtyUnits, status: "New", auditBy: "", auditDateTime: null, auditNote: "", returnBy: "", returnDateTime: null, returnNote: "", dateTime: jordanNowIso(), createdBy: APP.currentRole, updatedBy: APP.currentRole
    });
...
async function registerQuickPrescription() {
  ...
  const duplicates = findDuplicatePrescriptions(payload);
  if (duplicates.length) {
    openDuplicateWarningModal(payload, duplicates);
    return;
  }
...
async function saveSettings() {
  const nextSettings = {
    ...
    year: Number(q("settingsYear").value || jordanNowYear()),
    updatedAt: serverTimestamp()
  };
...
async function openResetPasswordModal() {
  if (APP.currentRole !== "ADMIN") return;
  q("resetPasswordPharmacy").value = WORK_PHARMACIES[0];
  openModal("resetPasswordModal");
}

async function resetSelectedPharmacyPassword() {
  if (APP.currentRole !== "ADMIN") return;
  const pharmacyName = q("resetPasswordPharmacy").value;
  const userEntry = Object.entries(USERS).find(([, user]) => user.displayName === pharmacyName);
  if (!userEntry) {
    showActionModal("Reset Password", "Selected pharmacy user was not found.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  const [userId] = userEntry;
  showActionModal("Reset Password", `Please wait while the password for ${pharmacyName} is being reset...`);
  ...
  finishActionModal(true, `Password for ${pharmacyName} was reset to ${DEFAULT_PASSWORD}.`);
}
...
async function savePharmacist() {
  ...
  const duplicate = APP.cache.pharmacists.find(p => p.active !== false && p.jobNumber === jobNumber && p.id !== APP.pharmacistEditId);
  if (duplicate) {
    showActionModal("Validation", "This job number already exists. Pharmacist was not saved.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  showActionModal(APP.pharmacistEditId ? "Update Pharmacist" : "Save Pharmacist", "Please wait while the pharmacist record is being saved...");
  const payload = {
    name,
    jobNumber,
  ...
  if (APP.pharmacistEditId) {
    ...
    await addDoc(collection(db, "pharmacists"), { ...payload, createdAt: serverTimestamp() });
  }
...
function renderDrugRows() {
  ...
  q("drugRxTbody").innerHTML = prescriptionScopeRows().filter(row => row.drugId === APP.selectedDrugId).map(row => {
    ...
    <tr>
      ...
      <td>
        <div class="audit-action-group vertical-edit-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn ${allowed ? "" : "hidden"}" data-edit-btn data-id="${row.id}">Edit</button><div class="edit-countdown" data-edit-countdown="${row.id}"></div>`}
          ${APP.currentRole === "ADMIN" ? `<button class="mini-danger-btn mini-btn delete-rx-btn" data-id="${row.id}">Delete</button>` : ""}
        </div>
      </td>
    </tr>`;
  }).join("") || `<tr><td colspan="10" class="empty-state">No prescriptions for this drug.</td></tr>`;
  renderLiveClocks();
}
  ...
  <html>
    <head>
      ...
      <style>
        ...
        .report-sub-row td{background:#f8fbff;font-size:11px;color:#334a62}
...
function reportStatusLine(row) {
  const status = row.status || "New";
  if (status === "Returned") {
    const by = row.returnBy || row.auditBy || "";
    const at = row.returnDateTime ? formatJordanDateTime(row.returnDateTime) : row.auditDateTime ? formatJordanDateTime(row.auditDateTime) : "";
    const note = row.returnNote || row.auditNote || "";
    return `Status: ${status}${by ? ` · By: ${by}` : ""}${at ? ` · Time: ${at}` : ""}${note ? ` · Note: ${note}` : ""}`;
  }
  const by = row.auditBy || "";
  const at = row.auditDateTime ? formatJordanDateTime(row.auditDateTime) : "";
  const note = row.auditNote || "";
  return `Status: ${status}${by ? ` · By: ${by}` : ""}${at ? ` · Time: ${at}` : ""}${note ? ` · Note: ${note}` : ""}`;
}

function reportAuditLine(row) {
  if (row.status === "Returned") {
    const by = row.returnBy || row.auditBy || "";
    const at = row.returnDateTime ? formatJordanDateTime(row.returnDateTime) : row.auditDateTime ? formatJordanDateTime(row.auditDateTime) : "";
    const note = row.returnNote || row.auditNote || "";
    return `${by ? `By: ${by}` : "-"}${at ? ` · Time: ${at}` : ""}${note ? ` · Note: ${note}` : ""}`;
  }
  if (!row.auditBy && !row.auditDateTime && !row.auditNote) return row.status === "New" ? "Status: New" : `Status: ${row.status || "New"}`;
  return `${row.auditBy ? `By: ${row.auditBy}` : "-"}${row.auditDateTime ? ` · Time: ${formatJordanDateTime(row.auditDateTime)}` : ""}${row.auditNote ? ` · Note: ${row.auditNote}` : ""}`;
}
      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr class="report-sub-row">
              <td colspan="7"><strong>${esc(reportStatusLine(row))}</strong></td>
            </tr>
...
function printComprehensiveReport() {
  ...
  const body = orderedSections.map((pharmacy, sectionIndex) => {
    ...
    const prescriptions = pharmacy === "General Hospital Stock"
      ? []
      : scopedPrescriptionRowsByPharmacy(pharmacy).filter(row => (!from || formatJordanDateTime(row.dateTime).slice(0, 10) >= from) && (!to || formatJordanDateTime(row.dateTime).slice(0, 10) <= to));
              ...
              <table>
                ...
                <tbody>
                  ${group.rows.map(row => `
                    ...
                    <tr class="report-sub-row">
                      <td colspan="7"><strong>Status:</strong> ${esc(row.status || "New")} &nbsp; | &nbsp; <strong>Audit:</strong> ${esc(reportAuditLine(row))}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
...
document.addEventListener("click", event => {
  ...
  const editBtn = event.target.closest("[data-edit-btn], .edit-rx-btn");
  if (editBtn) {
    closeAuditMenuPortal();
...
if (q("openResetPasswordBtn")) q("openResetPasswordBtn").onclick = openResetPasswordModal;
if (q("confirmResetPasswordBtn")) q("confirmResetPasswordBtn").onclick = resetSelectedPharmacyPassword;
q("logoutBtn").onclick = () => {
  q("userMenuDropdown").classList.add("hidden");
  APP.listeners.forEach(unsub => unsub && unsub());
  APP.listeners = [];
