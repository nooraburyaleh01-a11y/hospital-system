...
function formatJordanDateTime(value, withSeconds = false) {
  ...
  const simple = normalized.match(/^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2})(?::(\d{2}))?$/);
  if (simple) {
    const seconds = simple[3] || "00";
    return `${simple[1]} ${simple[2]}${withSeconds ? `:${seconds}` : ""}`;
  }
  const date = new Date(s);
  if (Number.isNaN(date.getTime())) return normalized.slice(0, withSeconds ? 19 : 16);
    ...
    <tr>
      ...
      <td>
        <div class="audit-action-group vertical-edit-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn" data-edit-btn data-id="${row.id}">Edit</button>`}
