      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr class="detail-row">
              <td colspan="7"><span class="detail-line">Status:</span> ${esc(row.status || "New")}${row.auditBy || row.auditDateTime || row.returnBy || row.returnDateTime ? ` &nbsp; | &nbsp; <span class="detail-line">Details:</span> <span class="detail-muted">${esc(row.status === "Returned" ? `${row.returnBy || ""}${row.returnDateTime ? ` • ${formatJordanDateTime(row.returnDateTime)}` : ""}` : `${row.auditBy || ""}${row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : ""}`)}</span>` : ""}</td>
