...
function msUntilEditExpires(_rx) {
  return Infinity;
}

function formatCountdown(_ms) {
...
function renderStaticOptions() {
  ...
  populateResetPasswordPharmacyOptions();
...
function renderSettings() {
  ...
  if (q("resetPasswordPharmacy")) q("resetPasswordPharmacy").value = APP.cache.settings.pharmacyType || "In-Patient Pharmacy";
...
function populateResetPasswordPharmacyOptions() {
  const select = q("resetPasswordPharmacy");
  if (!select) return;
  select.innerHTML = WORK_PHARMACIES.map(name => `<option value="${esc(name)}">${esc(name)}</option>`).join("");
}

async function resetSelectedPharmacyPassword() {
  if (APP.currentRole !== "ADMIN") return;
  const pharmacy = q("resetPasswordPharmacy")?.value;
  const role = PHARMACY_TO_USER_ROLE[pharmacy];
  if (!pharmacy || !role) {
    showActionModal("Reset Password", "Please select a pharmacy.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  showActionModal("Reset Password", `Please wait while the password for ${pharmacy} is being reset...`);
    ...
    <tr>
      ...
      <td>
        <div class="audit-action-group vertical-edit-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn" data-edit-btn data-id="${row.id}">Edit</button><div class="edit-countdown hidden" data-edit-countdown="${row.id}"></div>`}
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr class="report-sub-row">
              <td colspan="7"><strong>Status:</strong> ${esc(row.status || "Registered")}${row.status === "Returned" ? "" : `${row.auditBy || row.auditDateTime ? ` &nbsp; | &nbsp; <strong>Audit:</strong> ${esc(row.auditBy || "-")}${row.auditDateTime ? ` • ${esc(formatJordanDateTime(row.auditDateTime))}` : ""}` : ""}`}</td>
            </tr>
              ...
              <table>
                ...
                <tbody>
                  ${group.rows.map(row => `
                    ...
                    <tr class="report-sub-row">
                      <td colspan="6"><strong>Status:</strong> ${esc(row.status || "Registered")}${row.status === "Returned" ? "" : `${row.auditBy || row.auditDateTime ? ` &nbsp; | &nbsp; <strong>Audit:</strong> ${esc(row.auditBy || "-")}${row.auditDateTime ? ` • ${esc(formatJordanDateTime(row.auditDateTime))}` : ""}` : ""}`}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
...
document.addEventListener("click", event => {
  ...
  const editBtn = event.target.closest(".edit-rx-btn, [data-edit-btn]");
  if (editBtn) {
    closeAuditMenuPortal();
...
q("openResetPasswordModalBtn") && (q("openResetPasswordModalBtn").onclick = () => { populateResetPasswordPharmacyOptions(); q("resetPasswordPharmacy").value = q("settingsPharmacy").value || "In-Patient Pharmacy"; openModal("resetPasswordModal"); });
q("confirmResetPasswordBtn") && (q("confirmResetPasswordBtn").onclick = resetSelectedPharmacyPassword);
