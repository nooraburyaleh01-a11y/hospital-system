...
function syncAdminScopePreview() {
  if (APP.currentRole !== "ADMIN") return;
  const selected = q("settingsPharmacy")?.value || APP.cache.settings.pharmacyType || "In-Patient Pharmacy";
  APP.cache.settings = { ...APP.cache.settings, pharmacyType: selected };
  refreshScopedSelectors();
  renderControlPanel();
  renderDashboard();
  renderInventory();
...
function openResetPasswordModal() {
  ...
  q("resetPasswordPharmacy").value = q("settingsPharmacy")?.value || APP.cache.settings.pharmacyType || WORK_PHARMACIES[0];
  openModal("resetPasswordModal");
}

async function resetSelectedPharmacyPassword() {
  const pharmacy = q("resetPasswordPharmacy").value;
  const roleEntry = Object.entries(USERS).find(([, data]) => data.displayName === pharmacy);
  if (!roleEntry) {
    showActionModal("Reset Password", "Could not identify the selected pharmacy account.", false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  const [roleId] = roleEntry;
  showActionModal("Reset Password", `Resetting password for ${pharmacy}...`);
  const hash = await sha256(DEFAULT_PASSWORD);
  await updateDoc(doc(db, "users", roleId), {
    passwordHash: hash,
    mustChangePassword: true,
    updatedAt: serverTimestamp()
  });
  closeModal("resetPasswordModal");
  finishActionModal(true, `Password for ${pharmacy} has been reset to ${DEFAULT_PASSWORD}.`);
}
    ...
    <tr>
      ...
      <td>
        <div class="audit-action-group vertical-edit-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn edit-rx-btn ${allowed ? "" : "hidden"}" data-edit-btn data-id="${row.id}">Edit</button>`}
      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr>
              <td colspan="7"><strong>Status:</strong> ${esc(row.status || "New")}${row.status === "Returned" ? "" : `${row.auditBy || row.auditDateTime ? ` &nbsp; | &nbsp; <strong>Audit:</strong> ${esc(row.auditBy || "-")}${row.auditDateTime ? ` • ${esc(formatJordanDateTime(row.auditDateTime))}` : ""}` : ""}`}</td>
            </tr>
...
q("settingsPharmacy").onchange = syncAdminScopePreview;
