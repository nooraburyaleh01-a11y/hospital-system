...
function refreshScopedSelectors() {
  ...
  if (q("shipmentLocation")) q("shipmentLocation").value = scope;
}
    ...
    <tr>
      ...
      <td>
        <div class="audit-action-group vertical-edit-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn edit-rx-btn" data-edit-btn data-id="${row.id}" type="button">Edit</button><div class="edit-countdown hidden" data-edit-countdown="${row.id}"></div>`}
  ...
  <html>
    <head>
      ...
      <style>
        ...
        .detail-row td{background:#f9fbff;font-size:11px;color:#51657c}
        .detail-line{font-weight:700;color:#173a66}
        .detail-muted{color:#6b7d92}
...
function buildStatusLine(row) {
  if (!row) return "-";
  const parts = [];
  parts.push(`Status: ${row.status || "New"}`);
  if (row.status === "Returned") {
    if (row.returnBy) parts.push(`Returned By: ${row.returnBy}`);
    if (row.returnDateTime) parts.push(`Returned At: ${formatJordanDateTime(row.returnDateTime, true)}`);
    if (row.returnNote) parts.push(`Return Note: ${row.returnNote}`);
  } else {
    if (row.auditBy) parts.push(`Audited By: ${row.auditBy}`);
    if (row.auditDateTime) parts.push(`Audit Time: ${formatJordanDateTime(row.auditDateTime, true)}`);
    if (row.auditNote) parts.push(`Audit Note: ${row.auditNote}`);
  }
  return parts.join(" • ");
}
      ...
      <table>
        ...
        <tbody>
          ${rows.map(row => `
            ...
            <tr>
              <td colspan="7"><strong>Prescription Status:</strong> ${esc(buildStatusLine(row))}</td>
            </tr>
        ...
        ${pharmacy === "General Hospital Stock" ? "" : `
          ...
          ${prescriptionGroups.map((group, groupIndex) => `
            <div class="group ${groupIndex ? 'page-break' : ''}">
              ...
              <table>
                <thead><tr><th>Date & Time</th><th>Patient</th><th>File No.</th><th>Boxes</th><th>Units</th><th>Doctor</th><th>Status</th></tr></thead>
                <tbody>
                  ${group.rows.map(row => `
                    <tr><td>${esc(formatJordanDateTime(row.dateTime))}</td><td>${esc(row.patientName || "")}</td><td>${esc(row.fileNumber || "")}</td><td>${Number(row.qtyBoxes || 0)}</td><td>${Number(row.qtyUnits || 0)}</td><td>${esc(row.doctorName || "")}</td><td>${esc(row.status || "")}</td></tr>
                    <tr class="detail-row"><td colspan="7"><span class="detail-line">Audit Details:</span> <span class="detail-muted">${esc(row.status === 'Returned' ? '-' : ((row.auditBy || '') + (row.auditDateTime ? ` • ${formatJordanDateTime(row.auditDateTime)}` : '') || '-'))}</span></td></tr>`).join("")}
                </tbody>
              </table>
            </div>
...
q("openResetPasswordModalBtn").onclick = () => openModal("resetPasswordModal");
q("confirmResetPasswordBtn").onclick = resetPharmacyPassword;
q("logoutBtn").onclick = () => {
  q("userMenuDropdown").classList.add("hidden");
  APP.listeners.forEach(unsub => unsub && unsub());
  APP.listeners = [];
...
q("openResetPasswordModalBtn").onclick = () => { q("resetPasswordPharmacy").value = q("settingsPharmacy").value || "In-Patient Pharmacy"; openModal("resetPasswordModal"); };
