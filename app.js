...
const EDIT_WINDOW_HOURS = 8;
const DUPLICATE_LOOKBACK_DAYS = 30;

function jordanParts(dateInput = new Date()) {
...
function jordanDateKey(dateInput = new Date()) {
  const p = jordanParts(dateInput);
  return `${p.year}-${p.month}-${p.day}`;
}

function jordanMonthKey(dateInput = new Date()) {
  const p = jordanParts(dateInput);
  return `${p.year}-${p.month}`;
}

function formatJordanDateTime(dateInput) {
  if (!dateInput) return "";
  const p = jordanParts(dateInput);
  return `${p.year}-${p.month}-${p.day} ${p.hour}:${p.minute}`;
}

function nowJordanDisplay() {
  return formatJordanDateTime(new Date());
}

function todayKey() {
  return jordanDateKey(new Date());
}

const monthKey = dateText => jordanMonthKey(dateText || new Date());

function hoursSince(dateInput) {
  return (Date.now() - new Date(dateInput).getTime()) / 36e5;
}

function getEditLockInfo(rx) {
  const createdMs = new Date(rx?.dateTime || rx?.createdAt || Date.now()).getTime();
  const lockAtMs = createdMs + EDIT_WINDOW_HOURS * 60 * 60 * 1000;
  const remainingMs = lockAtMs - Date.now();
  return {
    isAdminOverride: APP.currentRole === "ADMIN",
    canEdit: APP.currentRole === "ADMIN" || remainingMs > 0,
    remainingMs,
    createdMs,
    lockAtMs
  };
...
function currentScopePharmacy() {
  ...
  if (APP.currentUser?.activePharmacy && APP.currentUser.pharmacyScope?.includes(APP.currentUser.activePharmacy)) return APP.currentUser.activePharmacy;
...
function renderStaticOptions() {
  ...
  const scope = currentScopePharmacy();
  const pharmacists = APP.cache.pharmacists.filter(p => (p.pharmacies || []).includes(scope) || (p.workplace || p.pharmacies?.[0]) === scope);
  ...
  q("pharmacistPharmacy").innerHTML = WORK_PHARMACIES.map(name => `<option value="${esc(name)}">${esc(name)}</option>`).join("");
...
function renderDashboard() {
  ...
  const todayPrescriptions = scopedPrescriptions.filter(p => jordanDateKey(p.dateTime || p.createdAt) === todayKey());
  const cardSearch = (q("drugCardsSearch").value || "").toLowerCase();

  q("dashboardScopeChip").textContent = `${scope} · Jordan time ${nowJordanDisplay()}`;
  q("metricRegistered").textContent = todayPrescriptions.filter(p => !["Pending", "Returned"].includes(p.status || "New")).length;
  q("metricPending").textContent = todayPrescriptions.filter(p => (p.status || "") === "Pending").length;
  q("metricReturned").textContent = todayPrescriptions.filter(p => (p.status || "") === "Returned").length;
  ...
  q("recentList").innerHTML = scopedPrescriptions.slice(0, 7).map(p => {
    ...
    return `
      <div class="recent-row">
        ...
        <div class="subline">${esc(formatJordanDateTime(p.dateTime || p.createdAt))}</div>
...
function renderSettings() {
  ...
  q("pharmacistsTbody").innerHTML = APP.cache.pharmacists.map(p => `
    <tr>
      <td>${esc(p.name)}</td>
      <td>${esc((p.pharmacies || [p.workplace]).filter(Boolean).join(", "))}</td>
...
async function registerQuickPrescription() {
  ...
  const normalizedFile = normalizeName(fileNumber);
  const thirtyDaysAgo = Date.now() - DUPLICATE_LOOKBACK_DAYS * 24 * 60 * 60 * 1000;
  const duplicateRows = APP.cache.prescriptions
    .filter(row => row.drugId === drugId)
    .filter(row => WORK_PHARMACIES.includes(row.pharmacy))
    .filter(row => new Date(row.dateTime || row.createdAt || 0).getTime() >= thirtyDaysAgo)
    .filter(row => normalizeName(row.patientName) === normalizedPatient || normalizeName(row.fileNumber) === normalizedFile)
    .sort((a, b) => String(b.dateTime || "").localeCompare(String(a.dateTime || "")));

  if (duplicateRows.length) {
    APP.pendingDuplicateApproval = {
      drugId, patientName, fileNumber, doctorName, pharmacistName, qtyBoxes, qtyUnits
    };
    q("duplicateAlertText").innerHTML = `Warning: this patient already received <strong>${esc(drug?.tradeName || "")}</strong> within the last ${DUPLICATE_LOOKBACK_DAYS} days.`;
    q("duplicateHistoryWrap").classList.add("hidden");
    q("duplicateHistoryTbody").innerHTML = duplicateRows.map(row => {
      const rowDrug = APP.cache.drugs.find(d => d.id === row.drugId);
      return `<tr>
        <td>${esc((rowDrug?.tradeName || "") + " " + (rowDrug?.strength || ""))}</td>
        <td>${esc(row.patientName || "")}</td>
        <td>${esc(row.fileNumber || "")}</td>
        <td>${esc(formatJordanDateTime(row.dateTime || row.createdAt))}</td>
      </tr>`;
    }).join("");
    openModal("duplicateAlertModal");
    return;
  }

  await finalizeQuickPrescription({ drugId, patientName, fileNumber, doctorName, pharmacistName, qtyBoxes, qtyUnits });
}

async function finalizeQuickPrescription(payload) {
  ...
  try {
    ...
    q("quickDrug").value = "";
    q("quickPharmacist").value = "";
    updateQuickAvailableStock();
    finishActionModal(true, `Prescription registered successfully at Jordan time ${nowJordanDisplay()}.`);
  } catch (error) {
...
async function savePharmacist() {
  ...
  const selectedPharmacies = [...q("pharmacistPharmacy").selectedOptions].map(opt => opt.value);
  const workplace = selectedPharmacies[0] || "";
  if (!name || !selectedPharmacies.length) {
  ...
  [...q("pharmacistPharmacy").options].forEach(opt => opt.selected = false);
...
function renderDrugRows() {
  ...
  q("drugRxTbody").innerHTML = prescriptionScopeRows().filter(row => row.drugId === APP.selectedDrugId).map(row => {
    const lock = getEditLockInfo(row);
    const editLabel = lock.isAdminOverride ? "Edit" : (lock.canEdit ? `Edit (${formatRemaining(lock.remainingMs)})` : "Locked");
    const editDisabled = row.status === "Returned" || (!lock.canEdit && APP.currentRole !== "ADMIN");
    return `
    <tr>
      <td>${esc(formatJordanDateTime(row.dateTime || row.createdAt))}</td>
      <td>${esc(row.patientName || "")}</td>
      ...
      <td>
        <div class="audit-action-group">
          ${row.status === "Returned" ? "-" : `<button class="primary-btn mini-btn edit-rx-btn ${editDisabled ? 'locked-btn' : ''}" data-id="${row.id}" ${editDisabled ? 'disabled' : ''}>${esc(editLabel)}</button>`}
...
function openEditPrescription(id) {
  const rx = APP.cache.prescriptions.find(row => row.id === id);
  if (!rx) return;
  const lock = getEditLockInfo(rx);
  if (!lock.canEdit && APP.currentRole !== "ADMIN") {
    showActionModal("Edit Locked", `This prescription can no longer be edited. The 8-hour edit window expired at ${formatJordanDateTime(lock.lockAtMs)} Jordan time.`, false);
    q("actionOkBtn").classList.remove("hidden");
    return;
  }
  ...
  q("editLockInfo").textContent = APP.currentRole === "ADMIN"
    ? `Admin override enabled. Original registration time: ${formatJordanDateTime(rx.dateTime || rx.createdAt)} Jordan time.`
    : `Time remaining for edit: ${formatRemaining(lock.remainingMs)}. Window closes at ${formatJordanDateTime(lock.lockAtMs)} Jordan time.`;
    ...
    <body>
      <div class="report">
        <div class="head">
          ...
          <div class="pill">Printed ${esc(nowJordanDisplay())} Jordan time</div>
        </div>
        ${bodyHtml}
      </div>
...
q("duplicateShowHistoryBtn").onclick = () => q("duplicateHistoryWrap").classList.toggle("hidden");
q("duplicateCancelBtn").onclick = () => { APP.pendingDuplicateApproval = null; closeModal("duplicateAlertModal"); };
q("duplicateContinueBtn").onclick = async () => {
  const pending = APP.pendingDuplicateApproval;
  APP.pendingDuplicateApproval = null;
  closeModal("duplicateAlertModal");
  if (pending) await finalizeQuickPrescription(pending);
};
